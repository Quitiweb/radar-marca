...
def _sample_results():
    return [
        CandidateDomain(domain="new.com", similarity_score=0.91, dns_resolves=True, http_reachable=True, risk_score=82, notes=["high_similarity"]),
        CandidateDomain(domain="seen.com", similarity_score=0.75, dns_resolves=True, http_reachable=False, risk_score=55, notes=["active_dns"]),
    ]
...
def test_to_markdown_and_html_include_history_sections():
    results = _sample_results()
    diff = SnapshotDiff(previous_path=None, current_path=None, new_domains=[results[0]], seen_domains=[results[1]], removed_domains=["gone.com"])
